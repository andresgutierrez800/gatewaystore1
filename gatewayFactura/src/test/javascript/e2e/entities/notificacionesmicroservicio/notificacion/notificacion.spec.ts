...
describe('Notificacion e2e test', () => {
  ...
  it('should load Notificacions', async () => {
    ...
    expect(await notificacionComponentsPage.getTitle()).to.eq('gatewayFacturaApp.notificacionesmicroservicioNotificacion.home.title');
    await browser.wait(
  ...
  it('should load create Notificacion page', async () => {
    ...
    expect(await notificacionUpdatePage.getPageTitle()).to.eq(
      'gatewayFacturaApp.notificacionesmicroservicioNotificacion.home.createOrEditLabel'
    );
  ...
  it('should create and save Notificacions', async () => {
    ...
    await promise.all([
      ...
      notificacionUpdatePage.setFechaEnviadoInput('01/01/2001' + protractor.Key.TAB + '02:30AM'),
      notificacionUpdatePage.setUserIdInput('5'),
      notificacionUpdatePage.tipoNotificacionSelectLastOption(),
      notificacionUpdatePage.formatodenotifiaccionSelectLastOption()
    ]);
    ...
    expect(await notificacionUpdatePage.getFechaEnviadoInput()).to.contain(
      '2001-01-01T02:30',
      'Expected fechaEnviado value to be equals to 2000-12-31'
    );
  ...
  it('should delete last Notificacion', async () => {
    ...
    expect(await notificacionDeleteDialog.getDialogTitle()).to.eq(
      'gatewayFacturaApp.notificacionesmicroservicioNotificacion.delete.question'
    );
