...
describe('Stock e2e test', () => {
  ...
  it('should load Stocks', async () => {
    ...
    expect(await stockComponentsPage.getTitle()).to.eq('gatewayFacturaApp.inventariomicroservicioStock.home.title');
  ...
  it('should load create Stock page', async () => {
    ...
    expect(await stockUpdatePage.getPageTitle()).to.eq('gatewayFacturaApp.inventariomicroservicioStock.home.createOrEditLabel');
  ...
  it('should create and save Stocks', async () => {
    ...
    await promise.all([stockUpdatePage.setCantidadInput('5'), stockUpdatePage.productSelectLastOption()]);
  ...
  it('should delete last Stock', async () => {
    ...
    expect(await stockDeleteDialog.getDialogTitle()).to.eq('gatewayFacturaApp.inventariomicroservicioStock.delete.question');
