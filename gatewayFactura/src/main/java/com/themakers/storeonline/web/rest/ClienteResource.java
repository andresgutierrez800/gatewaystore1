...
/**
 * REST controller for managing {@link com.themakers.storeonline.domain.Cliente}.
 */
@RestController
@RequestMapping("/api")
@Transactional
public class ClienteResource {
     ...
     */
    @GetMapping("/clientes")
    public List<Cliente> getAllClientes(@RequestParam(required = false) String filter) {
        if ("factura-is-null".equals(filter)) {
            log.debug("REST request to get all Clientes where factura is null");
            return StreamSupport
                .stream(clienteRepository.findAll().spliterator(), false)
                .filter(cliente -> cliente.getFactura() == null)
