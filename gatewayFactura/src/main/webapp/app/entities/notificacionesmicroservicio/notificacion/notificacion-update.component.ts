...
export class NotificacionUpdateComponent implements OnInit {
  ...
  editForm = this.fb.group({
    ...
    mensaje: [],
    fechaEnviado: [null, [Validators.required]],
    userId: [],
    tipoNotificacion: [],
    formatodenotifiaccion: []
  });
  ...
  ngOnInit(): void {
    this.activatedRoute.data.subscribe(({ notificacion }) => {
      if (!notificacion.id) {
        ...
        notificacion.fechaEnviado = today;
      }
  ...
  updateForm(notificacion: INotificacion): void {
    this.editForm.patchValue({
      ...
      fechaEnviado: notificacion.fechaEnviado ? notificacion.fechaEnviado.format(DATE_TIME_FORMAT) : null,
      userId: notificacion.userId,
      tipoNotificacion: notificacion.tipoNotificacion,
      formatodenotifiaccion: notificacion.formatodenotifiaccion
    });
  }
  ...
  private createFromForm(): INotificacion {
    return {
      ...
      fechaEnviado: this.editForm.get(['fechaEnviado'])!.value
        ? moment(this.editForm.get(['fechaEnviado'])!.value, DATE_TIME_FORMAT)
        : undefined,
      userId: this.editForm.get(['userId'])!.value,
      tipoNotificacion: this.editForm.get(['tipoNotificacion'])!.value,
      formatodenotifiaccion: this.editForm.get(['formatodenotifiaccion'])!.value
    };
  }
